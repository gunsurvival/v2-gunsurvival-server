...
let qtreeMap = new QuadTree(boundary, 4); // quadtree cho vật tĩnh (map)
for (let object of map) {
    if (object.type != "Bullet") {
        let point = new Point(object.pos.x, object.pos.y, object);
        qtreeMap.insert(point);
    }
}
...
myJob = setInterval(() => {
    ...
    let qtree = new QuadTree(boundary, 4); // quadtree cho những vật động
    ...
    let mapPoints = qtreeMap.query(range); // static point
    ...
    let points = qtree.query(range); // tìm các điểm mà mình va chạm với vật động
            ...
            switch (other.type) { // làm những việc sau khi bị va chạm
                ...
                case 'Bullet':
                    ...
                    if (blood <= 0) {
                        ...
                        for (let weapon of gunner.bag.arr)
                            drops.push(weapon);
                    }
